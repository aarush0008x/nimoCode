...
app.get('/api/users', async (req, res) => {
  if (isConnected && mongoDb) {
    try {
      // Auto-purge any mock users from MongoDB
      await mongoDb.collection('users').deleteMany({
        username: { $in: FAKE_USERNAMES }
      });

      const users = await mongoDb.collection('users').find({
        username: { $nin: FAKE_USERNAMES }
  ...
  const users = getCollection('users') || [];
  const realUsers = users.filter(u => u && u.username && !FAKE_USERNAMES.includes(u.username.toLowerCase()));
  const sorted = [...realUsers].sort((a, b) => (b.rating || 1200) - (a.rating || 1200)).map((u, i) => ({
    ...u,
    rank: i + 1,
    name: u.name || u.username,
    avatar: u.avatar || `https://api.dicebear.com/7.x/bottts/svg?seed=${encodeURIComponent(u.username)}`,
...
app.get('/api/reviews', async (req, res) => {
  const FAKE_REVIEW_IDS = ['rev-1', 'rev-2', 'rev-3', 'rev-4'];
  if (isConnected && mongoDb) {
    try {
      // Auto-purge any mock reviews from MongoDB
      await mongoDb.collection('reviews').deleteMany({
        $or: [
          { id: { $in: FAKE_REVIEW_IDS } },
          { _id: { $in: FAKE_REVIEW_IDS } },
          { username: { $in: FAKE_USERNAMES } }
        ]
      });
      const reviews = await mongoDb.collection('reviews').find({
        id: { $nin: FAKE_REVIEW_IDS },
        _id: { $nin: FAKE_REVIEW_IDS },
        username: { $nin: FAKE_USERNAMES }
      }).sort({ _id: -1 }).toArray();
  ...
  const clean = reviews.filter(r => r && !FAKE_REVIEW_IDS.includes(r.id) && !FAKE_REVIEW_IDS.includes(r._id) && !FAKE_USERNAMES.includes((r.username || '').toLowerCase()));
  res.json(clean);
});
